    ...
    return schema && <>
        ...
        <table >
            ...
            <tr>
                ...
                {schema[1].map(t => <th scope="col" key={t}>{constTypes.includes(t) ? <img width={75} height={"auto"} src={"/pokedoku-unlimited/"+t+".png"}  alt={t}/> : <span className={"schema-text"}>{t}</span> }</th>)}
            </tr>
            </thead>
            <tbody>
            {schema[0].map((type, i) => {
                return <tr key={type}>
                    <th scope="row">{constTypes.includes(type) ? <img width={75} height={"auto"} src={"/pokedoku-unlimited/"+type+".png"}  alt={type}/> : <span className={"schema-text"}>{type}</span>}</th>
                    <td>
                        {getCellContent(i, 0)}
                    </td>
                    <td>
